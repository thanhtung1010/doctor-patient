...
export class NumberOnlyDirective {
  ...
  @HostListener('keydown', ['$event']) onKeyDown(e: any) {
    ...
    let cursorPosition: number = e.target && e.target['selectionStart'] || 0;
    let originalValue: string = e.target && e?.target['value'] ? e.target.value : '';
  ...
  validateValue(value: string): void {
    ...
    let valid: boolean = (new RegExp(regex)).test(value) || false;
    let valueStr: string = Number(value).toString();
    if (this.max && Number(value) > this.max) {
      valueStr = this.max.toString();
    }
    this.hostElement.nativeElement['value'] = valid ? valueStr :
      (this.allowEmpty ? '' : this.min);
  }
