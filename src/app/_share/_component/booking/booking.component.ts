...
import { IBookingItem, IDoctorBookingInfor } from "app/inside/profile/_interfaces";
...
export class BookingComponent implements OnInit, OnChanges {
  ...
  @Input() doctorInfor: IDoctorBookingInfor | null = null;

  @Output() oSubmitSuccess = new EventEmitter();
  @Output() visibleBookingModalChange = new EventEmitter<boolean>();
  ...
  get bookingWithDoctor(): boolean {
    return this.doctorInfor !== null && _.size(this.doctorInfor) > 0
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['visibleBookingModal']) {
      if (changes['visibleBookingModal'].currentValue === true) {
        if (this.inputBookingInfor) {
          if (this.allowUpdate) this.getBookedAtList();
        } else if (this.bookingWithDoctor) {
          this.getDoctorBySpecialist();
        }
      }
    }
    if (changes['doctorInfor'] && this.doctorInfor !== null) {
      if (_.size(changes['doctorInfor'].currentValue) > 0) {
        this.getDoctorBySpecialist();
        this.bookingInfor.doctorId = this.doctorInfor.doctorId;
        this.bookingInfor.specialist = this.doctorInfor.specialist;
      }
    }
  }
  ...
  onEmitToggleBookingModal() {
    this.visibleBookingModalChange.emit(this.visibleBookingModal)
  }
